...
test.describe('useMultiBlockSelection E2E Tests', () => {
  ...
  test.describe('基础块选择功能', () => {
    ...
    test('应该能够清除所有选择', async ({ page }) => {
      ...
      await page.locator('.tiptap-editor p').nth(0).click({ modifiers: ['Control'] })
      await page.locator('.tiptap-editor p').nth(1).click({ modifiers: ['Control'] })
  ...
  test.describe('删除操作', () => {
    ...
    test('应该能够删除多个选中的块', async ({ page }) => {
      ...
      // 选择第一个和第三个段落
      await page.locator('.tiptap-editor p').nth(0).click({ modifiers: ['Control'] })
      await page.locator('.tiptap-editor p').nth(2).click({ modifiers: ['Control'] })
  ...
  test.describe('回调功能', () => {
    test('应该在选择变化时触发回调', async ({ page }) => {
      ...
      await page.click('.tiptap-editor')
      await page.type('.tiptap-editor', 'Test paragraph')
      await page.locator('.tiptap-editor p').first().click({ modifiers: ['Control'] })
  ...
  test.describe('最大选择数量限制', () => {
    test('应该遵守最大选择数量限制', async ({ page }) => {
      ...
      // 尝试选择所有 5 个段落
      for (let i = 0; i < 5; i++) {
        await page.locator('.tiptap-editor p').nth(i).click({ modifiers: ['Control'] })
      }

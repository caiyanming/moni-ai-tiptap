...
describe('向下拖拽失败分析', () => {
  ...
  describe('向上拖拽分析 (已知成功)', () => {
    it('应该正确将第三个段落拖拽到第一个段落上方', () => {
      ...
      const verifyResult = verifyDragResult(result.actualResult.beforeTexts, result.actualResult.afterTexts, 2, 0)
  ...
  describe('向下拖拽分析 (已知失败)', () => {
    it('应该正确将第一个段落拖拽到第三个段落下方', () => {
      ...
      console.log(
        '  拖拽前:',
        result.actualResult.beforeTexts.map((t, i) => `[${i}] ${t.slice(0, 20)}...`),
      )
      console.log(
        '  拖拽后:',
        result.actualResult.afterTexts.map((t, i) => `[${i}] ${t.slice(0, 20)}...`),
      )
      ...
      // 验证与DragTestHelper的一致性
      const verifyResult = verifyDragResult(result.actualResult.beforeTexts, result.actualResult.afterTexts, 0, 2)
  ...
  describe('索引计算逻辑验证', () => {
    it('验证向下拖拽的插入位置计算', () => {
      ...
      console.log(
        '  移除前:',
        beforeTexts.map((_, i) => i),
      )
      console.log(
        '  移除后:',
        afterTexts.map((_, i) => i),
      )

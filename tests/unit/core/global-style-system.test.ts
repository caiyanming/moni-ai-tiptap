...
describe('Global Style System', () => {
  ...
  describe('addGlobalStyleAttributes', () => {
    ...
    it('应该正确渲染HTML属性', () => {
      ...
      expect(attributes.moniGlobalFontFamily.renderHTML(testAttrs))
        .toEqual({ 'data-moni-global-font-family': '"Arial", serif' })
      expect(attributes.moniGlobalFontSize.renderHTML(testAttrs))
        .toEqual({ 'data-moni-global-font-size': '20' })
      expect(attributes.moniSemanticStyle.renderHTML(testAttrs))
        .toEqual({ 'data-moni-semantic-style': '{"color": "#333"}' })
      expect(attributes.moniStyleVersion.renderHTML(testAttrs))
        .toEqual({ 'data-moni-style-version': '3' })
    })
  })

...
async function globalTeardown() {
  ...
  try {
    ...
    // 保存测试总结
    fs.writeFileSync(path.join(testResultsDir, 'test-summary.json'), JSON.stringify(summaryReport, null, 2))
...
async function generateTestSummary(testResultsDir) {
  ...
  try {
    ...
    // 统计视频文件
    const videos = fs.readdirSync(testResultsDir).filter(file => file.endsWith('.webm'))
    summary.videos = videos.length

    // 统计trace文件
    const traces = fs.readdirSync(testResultsDir).filter(file => file.endsWith('.zip'))
    ...
    // 列出测试文件
    summary.testFiles = ['drag-handle.spec.js', 'stream-operations.spec.js', 'hidden-blocks.spec.js']
  } catch (error) {
...
async function cleanupTempFiles(testResultsDir) {
  try {
    // 清理临时文件（如果有的话）
    const tempFiles = fs.readdirSync(testResultsDir).filter(file => file.startsWith('temp-') || file.endsWith('.tmp'))

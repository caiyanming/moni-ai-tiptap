...
test.describe('🎨 拖拽手柄位置和指示器 UX 验证', () => {
  ...
  test('📏 验证不同屏幕尺寸下的手柄位置适配', async ({ page }) => {
    ...
    for (const size of testSizes) {
      ...
      if (paragraphBox && handleBox) {
        ...
        console.log(`✅ ${size.name}: 手柄位置 (${handleBox.x}, ${handleBox.y}), 尺寸 ${handleBox.width}x${handleBox.height}`)
      }
    ...
    if (secondBox) {
      ...
      try {
        ...
        if (indicatorBox) {
          ...
          console.log(`✅ 水平指示器位置验证通过: y=${indicatorBox.y}, width=${indicatorBox.width}, height=${indicatorBox.height}`)
        }
      } catch (error) {
    ...
    if (secondBox) {
      ...
      if (await indicators.count() > 0) {
        ...
        if (styles.backgroundColor.includes('rgb')) {
          const isBlueish = styles.backgroundColor.includes('59, 130, 246') || // #3b82f6
  ...
  test('⚡ 验证拖拽手柄的hover状态和动画效果', async ({ page }) => {
    ...
    expect(['grab', 'pointer', '-webkit-grab'].some(cursor => 
      hoverStyles.cursor.includes(cursor)
    )).toBe(true)
